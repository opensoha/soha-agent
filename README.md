...
```sh
go build -trimpath \
  -ldflags "-X github.com/opensoha/soha-agent/internal/agent/buildinfo.Version=v0.1.5 -X github.com/opensoha/soha-agent/internal/agent/buildinfo.Commit=$(git rev-parse --short HEAD) -X github.com/opensoha/soha-agent/internal/agent/buildinfo.Date=$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
...
```sh
make deploy-agent-image IMAGE_TAG=v0.1.5
```

The Hermes runner image can be built separately with:

```sh
make deploy-hermes-image IMAGE_TAG=v0.1.5
```
